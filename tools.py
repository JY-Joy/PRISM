...
def caption2embed_sd3(captions, tokenizer, text_encoder, device, weight_dtype, args=None, token_length=None, pad_clip=True):
    results = dict()
    ...
    if tokenizer[0] is not None:
        ...
        if pad_clip:
            ...
            results["encoder_hidden_states_clip_concat"] = encoder_hidden_states_clip_concat

    return results

def caption2embed_flux(captions, tokenizer, text_encoder, device, weight_dtype, args=None, token_length=None):
    results = dict()
    ...
    if tokenizer[0] is not None:
        ...
        input_ids_clip, attention_mask_clip = tokens_clip.input_ids, tokens_clip.attention_mask
        encoder_hidden_states_clip = text_encoder[0](
            input_ids_clip,
        ...
        for i in range(max(sentence_index) + 1):    # for each input caption
            ...
            # get pooled state
            encoder_hidden_states_clip_pooled = encoder_hidden_states_clip.pooler_output
            # get segments mean
            encoder_hidden_states_clip_pooled = encoder_hidden_states_clip_pooled[clip_sentence_index == i].mean(dim=0)
            # append one data to batch
            pooled_encoder_hidden_states_batch.append(encoder_hidden_states_clip_pooled)
        # Batchify
        results["encoder_hidden_states_clip_pooled"] = torch.stack(pooled_encoder_hidden_states_batch, dim=0)

    if tokenizer[1] is not None:
        tokens_t5 = tokenizer[1](
            captions,
            padding="max_length",
            max_length=token_length or tokenizer[1].model_max_length,
            truncation=True,
